function pickIndex(seed, day, count) {
...
function getVariantTitle(day, seed) {
  const index = pickIndex(seed, day, DAY_TITLES.length);
  return `Day ${day}: ${DAY_TITLES[index]}`;
}

function getVariantDetails({ day, destination, styleDesc, budgetNote, focusNote, seed }) {
  ...
  return [
    `Morning: ${morning.replace('the city', dest).replace('historic center', `${dest}'s historic center`)}`,
    `Afternoon: ${afternoon}`,
    `Evening: ${evening}`,
    tip,
    styleLine.trim(),
...
module.exports = { getVariantTitle, getVariantDetails, pickIndex };
